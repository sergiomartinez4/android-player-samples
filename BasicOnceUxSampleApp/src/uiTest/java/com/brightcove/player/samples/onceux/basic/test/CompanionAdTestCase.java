...
public class CompanionAdTestCase extends OnceUxUiAutomatorBase {
    ...
    public void testCompanionAdPresence() throws UiObjectNotFoundException, InterruptedException {
        ...
        skipAheadCompanionAdCheck(msecToMidroll, ADTYPE_MIDROLL);
        waitForEndOfAdCheck(ADTYPE_MIDROLL);
        skipAheadCompanionAdCheck(msecToPostroll, ADTYPE_POSTROLL);
    ...
    /**
     * Skips ahead, searching for the adOverlayTextView UiObject to be present, then asserts true for companionCheck. If
     * the wait for the ad break times out, then the test fails.
     *
    ...
    /**
     * Waits for adTextView UiObject to be present, then asserts true for companionCheck. If the wait for the ad break times
     * out, then the test fails.
     *
     * @param  int msec the desired number of milliseconds to wait before giving up on the next ad break loading
     * @param  String adType the string identifier for the specific adbreak the method plans to wait to. Only effects logcat.
     */
